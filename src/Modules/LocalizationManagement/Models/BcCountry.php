...
class BcCountry extends BaseBcModel {
...
//       "cca2","cca3"
    ...
    public function states(){
        return $this->hasMany(BcState::class, self::FK_ID);
    }
    ...
    public function cities(){
        return $this->hasManyThrough(
            BcCity::class, // le model/table qu'on veut retourner (cities)
            BcState::class, // le model/table intermédiare (states)
            self::FK_ID, // states.country_id == countries.id
            BcState::FK_ID, // cities.state_id = states.id
            $this->getKeyName(), // countries.id
            (new BcState)->getKeyName() // states.id
        );
    }
    ...
    public function quarters(){
        return $this->hasManyDeep(
            BcQuarter::class, // le model/table qu'on veut retourner (quarters)
            [
                BcState::class, // 1er Model Intermédiare
                BcCity::class // 2e Model intermédiare
            ...
            [
                ...
                (new BcState)->getKeyName(), // [PK] dans le 2e model intermédiare ("regions.id_region")
                (new BcCity)->getKeyName()  // [PK] dans le 3e model intermédiare ("towns.id_town").
            ]
